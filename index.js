...
client.on('ready', async () => {
    ...
    try {
        ...
        console.log('\nCampanha finalizada. Encerrando client.');
    ...
    for (let i = 0; i < contacts.length; i++) {
        ...
        try {
            if (CONFIG.checkIsRegistered) {
                ...
                if (!isRegistered) {
                    console.log(`[${i + 1}/${contacts.length}] ✖ ${numero} não tem WhatsApp — pulando.`);
                    appendLog(CONFIG.logFile, { numero, nome, status: 'PULADO', detalhe: 'numero nao registrado' });
                    falhas++;
                    continue;
                }
            }
            ...
            console.log(`[${i + 1}/${contacts.length}] ✔ Enviado para ${nome || numero} (${numero})`);
            appendLog(CONFIG.logFile, { numero, nome, status: 'ENVIADO' });
            enviados++;
        } catch (err) {
            console.error(`[${i + 1}/${contacts.length}] ✖ Erro ao enviar para ${numero}:`, err.message);
            appendLog(CONFIG.logFile, { numero, nome, status: 'ERRO', detalhe: err.message });
            falhas++;
        }
